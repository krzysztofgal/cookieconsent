    ...
    document.cookie.split(";").filter((item) => {
      if (item.indexOf("cconsent") >= 0) {
        ...
        if (typeof cookieData.version === "undefined") {
          ...
          if (
            cookieData.version !== window.CookieConsent.config.cookieVersion
          ) {
            return removeReload();
          }
        }
        ...
        for (let key in cookieData.categories) {
          ...
          if (
            typeof window.CookieConsent.config.categories[key] === "undefined"
          ) {
            return removeReload();
          }
        }
        ...
        for (let key in cookieData.consentMode) {
          if (
            typeof window.CookieConsent.config.consentModeControls[key] ===
            "undefined"
          ) {
            return removeReload();
          }
        ...
        cookieData.services.forEach(function (service) {
          ...
          if (
            typeof window.CookieConsent.config.services[service] === "undefined"
          ) {
            return removeReload();
          }
        });
