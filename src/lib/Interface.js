...
export default class Interface {
              ...
              el(
                ...
                el(
                  ...
                  window.CookieConsent.config.categories[key].checked
                    ...
                    : "×",
                  {
                    "aria-label": window.CookieConsent.config.categories[key]
                      .checked
                      ? Language.getTranslation(
                          window.CookieConsent.config,
                          window.CookieConsent.config.language.current,
  ...
  modalRedrawIcons() {
    ...
    for (let tabGroup of tabGroups) {
      ...
      if (
        window.CookieConsent.config.categories[tabGroup.dataset.category]
          .checked
      ) {
      ...
      that.render("bar", that.buildBar(), (bar) => {
        ...
        if (!window.CookieConsent.config.cookieExists) {
          setTimeout(() => {
            ...
            buttonConsentDecline !== null ??
              buttonConsentDecline.setAttribute("tabindex", "0");
            buttonConsentDecline !== null ??
              buttonConsentDecline.setAttribute("aria-hidden", "false");
  ...
  addEventListeners(elements) {
    ...
    for (let button of buttonConsentGive) {
      button.addEventListener("click", () => {
        ...
        buttonConsentDecline !== null ??
          buttonConsentDecline.setAttribute("tabindex", "-1");
        buttonConsentDecline !== null ??
          buttonConsentDecline.setAttribute("aria-hidden", "true");
        focusTarget.focus();
        modalOpen = false;
  ...
  writeBufferToDOM() {
    for (let action of window.CookieConsent.buffer.appendChild) {
      if (
        window.CookieConsent.config.categories[action.category].wanted === true
      ) {
  ...
  buildCookie(callback) {
    ...
    for (let key in window.CookieConsent.config.consentModeControls) {
      cookie.consentMode[key] = window.CookieConsent.config.categories[
        window.CookieConsent.config.consentModeControls[key]
      ]?.wanted
        ? "granted"
        : "denied";
    }
